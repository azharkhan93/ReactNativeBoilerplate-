...
import { VENDOR_STATS, MANAGEMENT_LINKS } from '@/components/Vendor/vendorProfileConstants';
...
export const ProfileScreen: React.FC<ProfileScreenProps> = ({ userRole, onNavigate }) => {
  ...
  const MODAL_CONFIG = MODAL_ITEMS.reduce((acc: Record<string, { title: string; content: React.ReactNode; height?: DimensionValue; scrollable?: boolean }>, item: ModalConfigItem) => {
    ...
    if (item.id === 'availability') content = <AvailabilityContent onClose={() => setModalType(null)} />;
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        <View className="px-5">
          <ProfileMenuSection
            ...
            items={(isVendor ? MANAGEMENT_LINKS : ACCOUNT_MENU_ITEMS) as ProfileMenuItemData[]}
