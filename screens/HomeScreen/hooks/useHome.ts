...
export const useHome = ({
  ...
  const { featuredServices, nearbyServices, recommendedServices } =
    useMemo(() => {
      ...
      const liveNearby: ServiceProductItem[] = vendorProfiles.map((v, idx) => ({
        ...
        isFavorite: idx % 2 === 0,
        category:
          v.categories?.[0]?.name ?? categoryNames[idx % categoryNames.length],
        imageUrl:
          v.imageUri ||
          'https://images.unsplash.com/photo-1520340356584-f9917d1eea6f',
      }));

      const liveFeatured: ServiceProductItem[] = vendorProfiles.map(
        (v, idx) => ({
          id: v.id,
          ...
          category:
            v.categories?.[0]?.name ??
            categoryNames[(idx + 1) % categoryNames.length],
          imageUrl:
            v.imageUri ||
            'https://images.unsplash.com/photo-1605610816744-13c4752fea01',
        }),
      );

      const liveRecommended: ServiceProductItem[] = vendorProfiles
        .slice(0, 3)
        .map((v, idx) => ({
          id: v.id,
          ...
          category:
            v.categories?.[0]?.name ??
            categoryNames[(idx + 2) % categoryNames.length],
          imageUrl:
            v.imageUri ||
            'https://images.unsplash.com/photo-1552930294-6b595f4c2974',
        }));
  ...
  const filteredRecommended = useMemo(
    () =>
      filterAndSortServices(
        filteredBySearch.recommendedServices,
        activeFilters,
      ),
