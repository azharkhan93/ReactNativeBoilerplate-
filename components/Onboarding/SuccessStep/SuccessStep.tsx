...
export const SuccessStep: React.FC<SuccessStepProps> = ({ onFinish }) => {
  return (
    <View
      style={{ width }}
      className="flex-1 items-center justify-between px-8 py-12 bg-[#030712]"
    >
      ...
      <View className="items-center">
        ...
        >
          ...
          <View className="w-48 h-48 rounded-full border-2 border-gray-800 items-center justify-center bg-gray-950 shadow-sm">
            <CheckCircle2 size={80} color="#22c55e" strokeWidth={1} />
          </View>
        </Animated.View>

        <Typography variant="h2" className="text-white mb-3 text-center">
        ...
        >
          Your account is all set. Browse top-rated providers and schedule your
          first professional service in minutes.
        </Typography>
      </View>
      ...
      <View className="w-full items-center">
        ...
        <Button onPress={onFinish} className="w-full mb-6" size="lg">
          <View className="flex-row items-center">
            <Typography
              variant="body-lg"
              className="text-white font-body-semibold mr-2"
            >
              Go to Home
            </Typography>
        ...
        <Typography
          variant="body-sm"
          className="text-slate-400 uppercase tracking-widest text-[10px]"
        >
    ...
    {Array.from({ length: total }).map((_, i) => (
      <View
        ...
        className={`h-1 rounded-full ${
          i === current ? 'w-8 bg-primary-500' : 'w-3 bg-gray-700'
        }`}
      />
    ))}
  </View>
