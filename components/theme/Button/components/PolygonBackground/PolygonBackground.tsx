...
import { PolygonBackgroundProps } from './types';

export const PolygonBackground: React.FC<PolygonBackgroundProps> = ({ variant, width, height }) => {
  const cornerSize = 12;

  // Path for a rectangle with clipped top-left and bottom-right corners
  const path = `
    M ${cornerSize} 0 
    H ${width} 
      ...
      <Svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        {variant === 'outlined' ? (
          <>
            ...
            {/* Border for outlined */}
            <Path d={path} fill="none" stroke="url(#borderGrad)" strokeWidth="2" />
          </>
        ) : (
