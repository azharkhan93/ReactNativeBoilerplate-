// Pure JS SHA-256 and HMAC-SHA-256 implementation for React Native
// Ref: FIPS PUB 180-4 (Secure Hash Standard)

function sha256(ascii: string): string {
  function rightRotate(value: number, amount: number) {
    return (value >>> amount) | (value << (32 - amount));
  }

  const words: number[] = [];
  const asciiLength = ascii.length;
  let i = 0;
  while (i < asciiLength) {
    words[i >> 2] |= (ascii.charCodeAt(i) & 0xff) << (24 - (i % 4) * 8);
    i++;
  }
  words[i >> 2] |= 0x80 << (24 - (i % 4) * 8);

  const blockCount = ((asciiLength + 8) >> 6) + 1;
  const paddingLength = blockCount * 16;
  while (words.length < paddingLength) {
    words.push(0);
  }
  words[paddingLength - 1] = asciiLength * 8;

  const hash = [
    0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c,
    0x1f83d9ab, 0x5be0cd19,
  ];

  const k = [
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1,
    0x923f82a4, 0xab1c5ed5, 0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
    0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174, 0xe49b69c1, 0xefbe4786,
    0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147,
    0x06ca6351, 0x14292967, 0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
    0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85, 0xa2bfe8a1, 0xa81a664b,
    0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a,
    0x5b9cca4f, 0x682e6ff3, 0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
    0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
  ];

  const w = new Array(64);
  for (let j = 0; j < words.length; j += 16) {
    let a = hash[0];
    let b = hash[1];
    let c = hash[2];
    let d = hash[3];
    let e = hash[4];
    let f = hash[5];
    let g = hash[6];
    let h = hash[7];

    for (let t = 0; t < 64; t++) {
      if (t < 16) {
        w[t] = words[j + t] | 0;
      } else {
        const s0 =
          rightRotate(w[t - 15], 7) ^
          rightRotate(w[t - 15], 18) ^
          (w[t - 15] >>> 3);
        const s1 =
          rightRotate(w[t - 2], 17) ^
          rightRotate(w[t - 2], 19) ^
          (w[t - 2] >>> 10);
        w[t] = (w[t - 16] + s0 + w[t - 7] + s1) | 0;
      }

      const s0_a = rightRotate(a, 2) ^ rightRotate(a, 13) ^ rightRotate(a, 22);
      const maj = (a & b) ^ (a & c) ^ (b & c);
      const t2 = (s0_a + maj) | 0;
      const s1_e = rightRotate(e, 6) ^ rightRotate(e, 11) ^ rightRotate(e, 25);
      const ch = (e & f) ^ (~e & g);
      const t1 = (h + s1_e + ch + k[t] + w[t]) | 0;

      h = g;
      g = f;
      f = e;
      e = (d + t1) | 0;
      d = c;
      c = b;
      b = a;
      a = (t1 + t2) | 0;
    }

    hash[0] = (hash[0] + a) | 0;
    hash[1] = (hash[1] + b) | 0;
    hash[2] = (hash[2] + c) | 0;
    hash[3] = (hash[3] + d) | 0;
    hash[4] = (hash[4] + e) | 0;
    hash[5] = (hash[5] + f) | 0;
    hash[6] = (hash[6] + g) | 0;
    hash[7] = (hash[7] + h) | 0;
  }

  let result = '';
  for (let j = 0; j < 8; j++) {
    const val = hash[j];
    const hex = (val >>> 0).toString(16);
    result += ('00000000' + hex).slice(-8);
  }
  return result;
}

export function hmacSha256(message: string, key: string): string {
  let keyBytes: number[] = [];
  if (key.length > 64) {
    const keyHash = sha256(key);
    for (let i = 0; i < keyHash.length; i += 2) {
      keyBytes.push(parseInt(keyHash.slice(i, i + 2), 16));
    }
  } else {
    for (let i = 0; i < key.length; i++) {
      keyBytes.push(key.charCodeAt(i));
    }
  }
  while (keyBytes.length < 64) {
    keyBytes.push(0);
  }

  const ipad = new Array<number>(64);
  const opad = new Array<number>(64);
  for (let i = 0; i < 64; i++) {
    ipad[i] = keyBytes[i] ^ 0x36;
    opad[i] = keyBytes[i] ^ 0x5c;
  }

  const ipadStr = String.fromCharCode(...ipad);
  const opadStr = String.fromCharCode(...opad);

  const innerHash = hexToBytes(sha256(ipadStr + message));
  const innerHashStr = String.fromCharCode(...innerHash);

  return sha256(opadStr + innerHashStr);
}

function hexToBytes(hex: string): number[] {
  const bytes: number[] = [];
  for (let c = 0; c < hex.length; c += 2) {
    bytes.push(parseInt(hex.slice(c, c + 2), 16));
  }
  return bytes;
}
